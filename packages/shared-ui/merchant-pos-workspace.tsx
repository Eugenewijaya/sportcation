      ...
      <div className="space-y-4 lg:col-span-2">
        <div className="rounded-3xl bg-white p-6 shadow-sm">
          <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-xl font-semibold text-[#1f2326]">Point of Sale</h2>
              <p className="mt-1 text-sm font-semibold text-muted-foreground">Add walk-in slots and items to cart</p>
            </div>
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full rounded-full border border-border bg-[#f9fbfb] py-2 pl-9 pr-4 text-sm font-semibold text-[#1f2326] outline-none focus:border-primary"
              />
            </div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {availableItems.map((item) => (
              <button
                ...
                className="flex items-center justify-between rounded-2xl border border-border p-4 text-left transition hover:border-primary hover:bg-[#f9fbfb]"
              >
                <div>
                  <span className="block text-sm font-bold text-[#1f2326]">{item.name}</span>
                  <span className="mt-1 block text-xs font-semibold text-muted-foreground">Rp {item.price.toLocaleString("id-ID")}</span>
                </div>
                <span className="grid h-8 w-8 place-items-center rounded-full bg-[#dcfff6] text-primary">
      ...
      <div className="space-y-4">
        <div className="rounded-3xl bg-white p-6 shadow-sm">
          <div className="mb-4 flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#f9fbfb] text-muted-foreground">
              <ShoppingCart className="h-5 w-5" />
            </span>
            <h3 className="text-lg font-semibold text-[#1f2326]">Current Cart</h3>
          </div>
              ...
              cart.map((item, i) => (
                <div key={i} className="flex items-center justify-between py-3">
                  <div className="min-w-0 pr-4">
                    ...
                    <p className="text-xs font-semibold text-muted-foreground">Rp {item.price.toLocaleString("id-ID")}</p>
                  </div>
          ...
          <div className="mt-4 border-t border-border pt-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-2xl font-semibold text-[#1f2326]">Rp {total.toLocaleString("id-ID")}</span>
            </div>
            ...
            <div className="mt-6 grid gap-2">
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl bg-primary py-3 text-sm font-bold text-white transition hover:bg-[#006650] disabled:opacity-50"
              >
              ...
              <button
                ...
                disabled={cart.length === 0 || isProcessing}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-white py-3 text-sm font-bold text-[#1f2326] transition hover:bg-[#f9fbfb] disabled:opacity-50"
              >
        ...
        {qrisUrl && (
          <div className="rounded-3xl bg-white p-6 shadow-sm flex flex-col items-center justify-center">
            ...
            <button 
              ...
              className="rounded-xl bg-primary px-4 py-2 text-white text-sm font-bold hover:bg-[#006650]"
            >
              Tutup & Selesai
            </button>
          </div>
