...
export const resells = sqliteTable(
  "resells",
  {
    ...
    price: integer("price").notNull(),
    status: text("status", { enum: ["active", "awaiting_payment", "sold", "cancelled"] }).notNull().default("active"),
...
export const auctions = sqliteTable(
  "auctions",
  {
    ...
    status: text("status", { enum: ["active", "awaiting_payment", "ended", "cancelled"] }).notNull().default("active"),
    createdAt: integer("created_at", { mode: "timestamp" }).notNull().default(sql`(strftime('%s', 'now'))`),
    updatedAt: integer("updated_at", { mode: "timestamp" }).notNull().default(sql`(strftime('%s', 'now'))`),
  },
  (table) => [
