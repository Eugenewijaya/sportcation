...
import { AlertCircle, ArrowLeft, ArrowRight, Eye, EyeOff, LoaderCircle, LockKeyhole, Mail, Store, UserRound } from "lucide-react"
...
export default function MerchantRegisterPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      // NOTE: the backend assigns the merchant_owner role via specific endpoint,
      // but initially it registers as a normal user. Or we can set role here if the authClient supports it.
      // Assuming a generic signup, and merchant onboarding happens inside the app, 
      // or we just sign up and redirect to /merchant/verification
      ...
      if (result.error) throw new Error(result.error.message || "Gagal membuat akun bisnis.")
  ...
  async function loginWithGoogle() {
    ...
    try {
      ...
      // If we redirect to /merchant/verification on success?
      // Better to rely on layout to redirect if not verified.
    } catch {
    ...
    <main className="flex min-h-screen bg-slate-50">
        ...
        <div className="relative flex h-full flex-col justify-between p-12">
          ...
          <div>
            <h1 className="text-4xl font-bold text-white leading-tight mb-4 tracking-tight">
              Maksimalkan Okupansi,<br/>Fokus ke Bisnis.
            </h1>
            <p className="text-lg text-slate-300">
              Bergabung dengan ratusan lapangan olahraga yang telah menggunakan sistem kami.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="flex flex-1 flex-col items-center justify-center px-6 py-12 lg:px-16">
        <div className="w-full max-w-sm">
          ...
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Daftar Mitra</h2>
          <p className="mt-2 text-sm text-slate-500">Buat akun pengelola untuk mendaftarkan lapangan Anda.</p>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="bg-slate-50 px-4 text-slate-500">Atau daftar dengan email</span>
            </div>
          </div>

          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Nama Pemilik / Manajer</label>
            ...
            >
              {submitting ? <LoaderCircle className="h-5 w-5 animate-spin" /> : "Buat Akun Bisnis"}
          ...
          <p className="mt-8 text-center text-sm text-slate-500">
            Sudah terdaftar?{" "}
            <Link href="/merchant/login" className="font-semibold text-blue-600 hover:text-blue-700">
              Masuk di sini
            </Link>
          </p>
        </div>
